PDA_UTP_AUTH_SEED = Buffer.from("ZEhiKcLS")
PDA_BANK_VAULT_SEED = Buffer.from("zE8d8R9G")
PDA_BANK_INSURANCE_VAULT_SEED = Buffer.from("uDMUkwVG")
PDA_BANK_FEE_VAULT_SEED = Buffer.from("PpqJY00S")

VERY_VERBOSE_ERROR = Buffer.from(
  ""
).toString("base64");

###
# @internal
###
COLLATERAL_DECIMALS = 6 # USDC decimals
