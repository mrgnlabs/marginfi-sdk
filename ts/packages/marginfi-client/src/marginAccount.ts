...
export class MarginAccount {
  ...
  async makeHandleBankruptcyIx() {
    const remainingAccounts = await this.getObservationAccounts();
    const insuranceVaultAuthorityPk = (await getBankAuthority(this._group.publicKey, this._program.programId, BankVaultType.InsuranceVault))[0]
  ...
  async getObservationAccounts(): Promise<AccountMeta[]> {
    return (
      await Promise.all(
        this.activeUtps().map(async (utp) => await utp.getObservationAccounts())
      )
    ).flatMap((a) => a);
  }
  ...
  async localObserve(): Promise<IndexedObservation[]> {
    const debug = require('debug')(
      `mfi:margin-account:${this.publicKey.toString()}:observe`
    );
  ...
  async getObserver(observations?: IndexedObservation[]): Promise<Uint8Array> {
    ...
    for (let observation of observations) {
      observer = inject_observation_into_observer(
        observer,
        observation.observation.toWasm(),
  ...
  async checkRebalance() {
    require('debug')(
      `mfi:margin-account:${this.publicKey.toString()}:rebalance`
    )('Checking rebalance');
  ...
  private async checkRebalanceWithdraw() {
    const debug = require('debug')(
      `mfi:margin-account:${this.publicKey.toString()}:rebalance:withdraw`
    );
    ...
    const rebalanceNeeded = rebalance_withdraw_valid(
    ...
    try {
      const sig = await this.utpFromIndex(
        richestUtpObservation.utp_index
      ).withdraw(decimalToNative(withdrawAmountDecimal));
  ...
  private async checkRebalanceDeposit() {
    let debug = require('debug')(
      `mfi:margin-account:${this.publicKey.toString()}:rebalance:deposit`
    );
    ...
    for (let indexed_observation of indexed_observations) {
      ...
      debug(
        'Trying to rebalance deposit UTP:%s amount %s (RBDA)',
        utp_index,
        cappedRebalanceAmount
      );

      try {
        let sig = await this.utpFromIndex(utp_index).deposit(
          cappedRebalanceAmount
        );
  ...
  async checkBankruptcy() {
    const debug = require('debug')(
      `mfi:margin-account:${this.publicKey.toString()}:bankruptcy`
    );
  ...
  private async loadAccountAndGroupAi(): Promise<AccountInfo<Buffer>[]> {
    ...
    debug(
      'Loading margin account %s, and group %s',
      this.publicKey,
      this._config.groupPk
    );
  ...
  public async liquidate(
    ...
    const debug = require('debug')(
      `mfi:margin-account:${this.publicKey.toString()}:liquidate`
    );
    let [bankAuthority, _] = await getBankAuthority(
      this._config.groupPk,
      this._program.programId
    );
    const remainingAccounts = await this.getObservationAccounts();
    ...
    debug(
      'Liquidator %s, liquidating UTP %s',
      marginAccountLiquidateePk,
      utpIndex
    );
  ...
  public async getMarginRequirement(
    ...
    let [marginGroupAi, marginAccountAi] = await this.loadAccountAndGroupAi();
    let mreq = get_margin_requirement(
      marginAccountAi.data,
      marginGroupAi.data,
      type
    );
