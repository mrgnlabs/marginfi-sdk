...
impl DoctorConfig {
    pub fn from_env() -> DoctorConfig {
        let marginfi_program = Pubkey::from_str(&std::env::var("MARGINFI_PROGRAM").unwrap()).unwrap();
        let marginfi_group = Pubkey::from_str(&std::env::var("MARGINFI_GROUP").unwrap()).unwrap();
        let keypair = {
            let wallet_path;

            if let Some(wallet_path) = std::env::var("WALLET").ok() {
                return read_keypair_file(wallet_path).unwrap();
            }

            if let Some(wallet_key) = std::env::var("WALLET_KEY").ok() {
        ...
        let cluster = Cluster::from_str(&std::env::var("ENV").unwrap()).unwrap();
...
impl GroupCache {
    pub fn new(program: Program, config: DoctorConfig, address_book: &AddressBook) -> Self {
        ...
        let (bank_authority, _) = Pubkey::find_program_address(
            &[PDA_BANK_VAULT_SEED, &marginfi_group.to_bytes()],
        ...
        let root_bank_ai =
            AccountInfo::from(&mut (root_bank_pk, rpc.get_account(&root_bank_pk).unwrap()));
        ...
        Self {
            marginfi_group: (marginfi_group, marginfi_group),
            bank_authority,
    ...
    pub fn reload(&mut self, program: Program, config: DoctorConfig, address_book: AddressBook) {
        *self = GroupCache::new(program, self.marginfi_group.0, config, ad);
    }
}
...
impl Doctor {
    pub fn new(config: DoctorConfig, rpc_url: String) -> Doctor {
        ...
        Self {
            ...
            client,
            program,
            cache: GroupCache::new(program, config, &address_book),
            address_book,
        }
    }
    ...
    fn task(&mut self) {
        ...
        accounts.iter().for_each(|(address, marginfi_account)| {
            ...
            ah.rebalance_if_needed();
        });
    }

    fn rebalance_if_needed(self, (mfi_account_pk, mfi_account): (&Pubkey, &MarginfiAccount)) {}
...
impl MarginAccountHandler {
                ...
                let mut utp_observe_accounts = match index {
                    ...
                    ZO_UTP_INDEX => vec![
                        ...
                        AccountMeta::new_readonly(
                            self.margin_account_cache.utp_zo_cache.unwrap().zo_control.0,
                            false,
                        ),
    ...
    fn get_utp_authority(self, utp_index: usize) -> (Pubkey, u8) {
        ...
        Pubkey::find_program_address(
            &[PDA_UTP_AUTH_SEED, &utp_config.authority_seed.to_bytes()],
            ...
            .for_each(|(index, _)| match index {
                MANGO_UTP_INDEX => {
                    ...
                    health_cache.init_vals_with_orders_vec(
                        ...
                        mango_cache,
                        mango_account,
                        &[Option::None::<Ref<serum_dex::state::OpenOrders>>; MAX_PAIRS],
                    );
                    ...
                    let deposit_amount =
                        get_max_rebalance_deposit_amount(&mut health_cache, &mango_group)
                            .unwrap()
                ...
                ZO_UTP_INDEX => {
                    ...
                    let (zo_margin_pk, zo_margin) =
                        &self.margin_account_cache.utp_zo_cache.unwrap().zo_margin;
                    let (zo_control_pk, zo_control) =
                        &self.margin_account_cache.utp_zo_cache.unwrap().zo_control;
